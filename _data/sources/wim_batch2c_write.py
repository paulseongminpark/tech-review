"""Write the summaries from wim_gen_0321.py that were defined but never written."""
import json, sys
sys.stdout.reconfigure(encoding='utf-8')

# Import the summaries dict from wim_gen_0321
import importlib.util, os
spec = importlib.util.spec_from_file_location("wim_gen_0321",
    os.path.join(os.path.dirname(__file__), "wim_gen_0321.py"))
# Can't import cleanly because it has a print at bottom. Just re-run its write logic.
# Instead, just call the file directly after removing the diagnostic print.
# Actually simpler: just define a minimal script that calls the summaries.
...
# Re-define all summaries from wim_gen_0321 here for writing
summaries = {}
  ...
  "sections": [
    {
      ...
      "highlights": ["이 영상의 자막은 제목과 전혀 다른 내용인 척추지압사 Dr. Jen의 목 통증 해결 가이드를 담고 있다.", "스마트폰과 컴퓨터 사용으로 인한 자세 불량이 목 통증의 주원인이다."],
      "quote": "The number one cause of neck pain is poor posture."
    },
    {
      "heading": "목 통증의 원인과 3가지 교정법",
      "body": "매일 수시간 동안 스마트폰, 컴퓨터, 책상 앞에 구부정하게 앉는 습관이 목 통증을 유발한다. 화면 높이를 눈높이에 맞추고 귀가 어깨 위에 직접 위치하도록 자세를 조정하는 것이 첫 번째 교정법이다. 목 근육 스트레칭, 인체공학적 환경 설정을 일상화하면 만성 통증을 예방할 수 있다.",
      "highlights": ["화면 높이를 눈높이에 맞추고 귀가 어깨 위에 직접 위치하도록 자세를 조정한다.", "목 근육 스트레칭, 인체공학적 환경 설정을 일상화하면 만성 통증을 예방할 수 있다."],
      "quote": None
    },
    {
      "heading": "예방의 중요성과 장기 습관화",
      "body": "통증이 없더라도 예방적 스트레칭과 자세 관리를 습관화하는 것이 장기적으로 훨씬 효과적이다. 작은 일상 습관의 변화가 만성 통증을 예방하는 가장 강력한 도구다. 장시간 개발 작업을 하는 환경에서 모니터 높이와 자세 관리는 생산성에도 직접 영향을 미친다.",
      "highlights": ["통증이 없더라도 예방적 스트레칭과 자세 관리를 습관화하는 것이 장기적으로 훨씬 효과적이다.", "작은 일상 습관의 변화가 만성 통증을 예방하는 가장 강력한 도구다."],
      "quote": None
    },
    {
      "heading": "자막 오염 사례로서의 의미",
      "body": "이 영상은 YouTube 자막이 영상 제목/내용과 전혀 다른 콘텐츠로 오염된 사례다. tech-review 블로그의 YouTube 인제스트 파이프라인에서 제목-내용 일치 검증이 필요하다는 것을 보여준다. 오염된 자막을 그대로 처리하면 잘못된 카테고리 분류와 검색 결과가 발생할 수 있다.",
      "highlights": ["이 영상은 YouTube 자막이 영상 제목/내용과 전혀 다른 콘텐츠로 오염된 사례다.", "tech-review 블로그의 YouTube 인제스트 파이프라인에서 제목-내용 일치 검증이 필요하다는 것을 보여준다."],
      "quote": None
    }
  ],
  "key_takeaways": [
    "자막 오염 영상 — 제목은 Claude 스킬, 내용은 척추지압 목 통증 교정 (Dr. Jen)",
    "인제스트 파이프라인에 제목-자막 일치 검증 단계 필요성을 보여주는 사례",
    "자세 불량이 목 통증 주원인 — 화면 높이 조정, 스트레칭, 인체공학 설정이 예방법"
  ],
  "tech_stack": [],
  "apply_points": [
    {"text": "tech-review YouTube 인제스트 파이프라인에 제목-내용 일치도 검증 단계 추가 필요 — 이 영상이 오염 콘텐츠 필터링의 필요성을 보여주는 실제 사례", "key": True},
    {"text": "WezTerm + tmux 장시간 세션 중 모니터 높이와 자세 점검 루틴을 개발 습관으로 추가 고려", "key": False}
  ],
  "smart_brevity": {
    "why": "자막 오염 사례. 콘텐츠 인제스트 파이프라인의 품질 보증 없이는 잘못된 데이터가 시스템에 축적된다.",
    "what": "자막 기준: 척추지압사 Dr. Jen의 목 통증 예방 3가지 교정법 (제목과 무관한 오염 콘텐츠).",
    "axioms": [
      {"label": "Yes, but", "content": "자세 교정 팁 자체는 유효하지만, 이 영상은 AI 도구 리뷰와 무관한 자막 오염 사례다."}
    ]
  }
}
  ...
  "sections": [
    {
      ...
      "body": "Miro의 MCP 서버를 Claude Code와 연결하면 로컬 코드베이스를 직접 분석하여 Miro 보드에 시스템 아키텍처 다이어그램을 자동으로 생성할 수 있다. Claude Code에서 /mcp 명령어로 code explain on board 프롬프트를 실행하면, Miro 보드 URL과 코드 경로만 지정해도 다이어그램이 자동으로 만들어진다. 결과물로 시스템 아키텍처 다이어그램, 요청 흐름 시퀀스 다이어그램, 미들웨어 및 인증 플로우차트, MCP 도구 마인드맵 등 4가지 시각화가 생성된다.",
      "highlights": ["Miro의 MCP 서버를 Claude Code와 연결하면 로컬 코드베이스를 직접 분석하여 Miro 보드에 시스템 아키텍처 다이어그램을 자동으로 생성할 수 있다.", "Miro 보드 URL과 코드 경로만 지정해도 다이어그램이 자동으로 만들어진다."],
      "quote": "I just ran Miro's MCP server with Claude to generate a diagram explaining our internal MCP, and this is the result."
    },
    {
      "heading": "생성된 다이어그램 4종: 실제 출력물",
      "body": "시스템 아키텍처 다이어그램은 AI 어시스턴트, 클라이언트, 서버, Redis 데이터 스토어, 플랫폼 API 간의 연결 관계를 범례와 함께 시각화한다. 요청 흐름 시퀀스 다이어그램은 각 컴포넌트 간 메시지 흐름을 시간 순서로 표현하고, 미들웨어 및 인증 플로우차트는 요청 처리 경로를 보여준다. MCP 도구 마인드맵은 사용 가능한 도구들의 계층 구조를 한눈에 파악하게 해준다.",
      "highlights": ["시스템 아키텍처 다이어그램은 AI 어시스턴트, 클라이언트, 서버, Redis 데이터 스토어, 플랫폼 API 간의 연결 관계를 시각화한다.", "MCP 도구 마인드맵은 사용 가능한 도구들의 계층 구조를 한눈에 파악하게 해준다."],
      "quote": None
    },
    {
      "heading": "지속적 문서화 파이프라인으로 활용",
    ...
    },
    {
      ...
      "body": "Claude Code에 Miro MCP 서버를 추가하는 것이 전제 조건이다. /mcp 명령어 실행 후 code explain on board 프롬프트를 선택하고, Miro 보드 URL과 분석할 코드 경로를 입력하면 Claude가 자동으로 코드를 분석하고 다이어그램을 생성한다. 처리 중 Claude가 작업하는 동안 대기하면 결과는 지정한 Miro 보드에 직접 나타난다.",
...
summaries["Kopack's White Day Gift. Free distribution of a Parallel Worktree Skill + Hook that you can use with Claude Code"] = {
  "sections": [
    {
      ...
      "body": "Git 브랜치는 같은 작업 디렉토리를 공유하지만 워크트리는 독립된 디렉토리에서 같은 저장소의 다른 브랜치를 동시에 체크아웃하여 작업할 수 있게 한다. 개발 브랜치 작업 중 메인 브랜치에 버그가 발견되었을 때 브랜치만으로는 작업 컨텍스트를 잃지 않고 빠르게 전환하기 어렵지만 워크트리는 독립된 공간에서 동시에 작업을 유지할 수 있다. 워크트리를 Claude Code와 결합하면 여러 독립된 작업을 병렬로 실행하면서 각 작업의 컨텍스트가 완전히 분리되어 오염 없이 진행된다.",
      "highlights": ["워크트리는 독립된 디렉토리에서 같은 저장소의 다른 브랜치를 동시에 체크아웃하여 작업할 수 있게 한다.", "워크트리를 Claude Code와 결합하면 여러 독립된 작업을 병렬로 실행하면서 각 작업의 컨텍스트가 완전히 분리된다."],
      "quote": "워크트리 만들고 독립된 공간에서 작업시키고 내가 아무것도 하지 않아도 자동으로 PR 만들어 주고."
    },
    {
      "heading": "병렬 워크트리 스킬 구조: 자동 PR 생성까지",
      "body": "Kopack이 공개한 병렬 워크트리 스킬은 워크트리 생성, 독립된 Claude Code 인스턴스에 작업 위임, 작업 완료 후 PR 자동 생성의 3단계를 자동화한다. 머지 워크트리 스킬을 함께 사용하면 PR 생성 후 메인 브랜치로의 머지까지 Claude Code가 자동으로 처리한다. 병렬 작업 중 어떤 작업이 어느 단계에 있는지 기억하지 않아도 되므로 인지 부하가 크게 줄어든다.",
      "highlights": ["병렬 워크트리 스킬은 워크트리 생성, 독립된 Claude Code 인스턴스에 작업 위임, 작업 완료 후 PR 자동 생성의 3단계를 자동화한다.", "병렬 작업 중 어떤 작업이 어느 단계에 있는지 기억하지 않아도 되므로 인지 부하가 크게 줄어든다."],
      "quote": None
    },
    {
      "heading": "Hook 자동화: 워크트리 전환 시 컨텍스트 복원",
      "body": "워크트리 전환 시 자동으로 실행되는 Hook이 해당 브랜치의 작업 목표, 현재 상태, 주의사항을 Claude Code에 자동으로 주입한다. 스킬과 Hook의 조합이 병렬 작업의 가장 큰 문제인 컨텍스트 손실을 구조적으로 해결한다. Kopack은 자신이 실제로 사용하는 스킬과 Hook 파일을 무료로 공개했다.",
      "highlights": ["Hook이 해당 브랜치의 작업 목표, 현재 상태, 주의사항을 Claude Code에 자동으로 주입한다.", "스킬과 Hook의 조합이 병렬 작업의 가장 큰 문제인 컨텍스트 손실을 구조적으로 해결한다."],
      "quote": None
    },
    {
      "heading": "무료 배포: 즉시 활용 가능한 실전 코드",
      "body": "공개된 파일을 그대로 사용하거나 자신의 프로젝트 구조에 맞게 수정하여 즉시 활용할 수 있다. 스킬 파일의 구조를 이해하면 유사한 패턴으로 다른 자동화 스킬을 직접 만들 수 있는 기반이 된다. 이 구조는 워크트리 관리의 복잡성을 완전히 추상화하여 병렬 작업에 집중할 수 있게 해준다.",
      "highlights": ["스킬 파일의 구조를 이해하면 유사한 패턴으로 다른 자동화 스킬을 직접 만들 수 있는 기반이 된다.", "이 구조는 워크트리 관리의 복잡성을 완전히 추상화하여 병렬 작업에 집중할 수 있게 해준다."],
      "quote": None
    }
  ],
  "key_takeaways": [
  ...
  "apply_points": [
    ...
    {"text": "현재 orchestration의 Hook 시스템과 Kopack의 워크트리 Hook 패턴을 비교하여 컨텍스트 주입 방식 개선 가능", "key": True},
...
# The remaining English videos (How to make Claude Code less dumb, Build Self-Improving, etc.)
# are already defined in wim_gen_0321.py's summaries dict
# Let me directly apply them here by loading the definitions

# How to make Claude Code less dumb - same as in wim_gen_0321
summaries["How to make Claude Code less dumb"] = {
  "sections": [
    {
      "heading": "Claude Code가 멍청하게 느껴지는 진짜 이유",
      "body": "20년 경력의 테크 빌더이자 VC인 저자는 Claude Code가 기대에 미치지 못한다고 느끼는 사람들이 실제로 Claude Code 자체의 문제가 아니라 잘못된 사용 방식 때문에 좌절한다고 주장한다. Claude Code는 단순히 코드를 생성하는 도구가 아니라 컨텍스트 엔지니어링이 필요한 시스템이다. 충분한 컨텍스트, 명확한 지시, 올바른 CLAUDE.md 설정이 없으면 Claude Code는 반복적인 실수를 저지르고 비효율적으로 작동한다.",
    ...
    },
    {
      ...
      "highlights": ["이 파일이 없거나 부실하면 Claude Code는 매번 처음부터 추론해야 하므로 일관성이 없고 비효율적이다.", "프로젝트별 CLAUDE.md에 기술 스택, 아키텍처 결정, 금지된 접근법을 명시하면 Claude Code의 출력 품질이 극적으로 향상된다."],
      "quote": None
    },
    {
  ...
  "apply_points": [
    {"text": "orchestration의 CLAUDE.md 구조를 프로젝트별 금지 패턴과 선호 접근법으로 더 정교하게 구성하는 것 검토 — 세션별 학습 루프 강화 가능", "key": True},
...
summaries["Build Self-Improving Claude Code Skills. The Results Are Crazy."] = {
  "sections": [
    ...
    },
    {
      ...
      "highlights": ["코드 리뷰 스킬은 반복적으로 놓치는 패턴을 추적하여 체크리스트를 자동으로 강화한다.", "문서화 스킬은 사용자 피드백을 반영하여 출력 형식을 지속적으로 개선하는 방식으로 진화한다."],
      "quote": None
    },
    {
      "heading": "구현 방법: 자기 평가 프롬프트 설계",
      "body": "자기 개선 루프를 만들기 위한 핵심은 스킬 마지막에 자기 평가 섹션을 추가하는 것이다. 이 섹션에서 Claude Code에게 방금 수행한 작업의 약점, 더 나은 접근 방법, 다음 실행을 위한 구체적 개선 사항을 분석하도록 지시한다. 이 분석 결과를 스킬 파일의 lessons learned 섹션에 자동으로 추가하도록 설계한다.",
      "highlights": ["스킬 마지막에 자기 평가 섹션을 추가하는 것이 자기 개선 루프의 핵심이다.", "이 분석 결과를 스킬 파일의 lessons learned 섹션에 자동으로 추가하도록 설계한다."],
      "quote": None
    },
    {
  ...
  "sections": [
    {
      ...
      "body": "Claude.ai의 Cowork는 마케팅 워크플로우 전반을 자동화하는 방식을 근본적으로 바꾸었다. 카피라이팅, 비주얼, 소셜 미디어 전략 등 마케팅의 거의 모든 부분을 Cowork 기반으로 자동화할 수 있게 되었다. Cowork는 Claude Chat, Claude Code와 함께 Claude 생태계의 세 번째 주요 인터페이스로, 특히 반복적이고 구조화된 작업에서 강점을 발휘한다.",
    ...
    {
      "heading": "Cowork 기반 마케팅 스킬 구축과 PPT 자동 생성",
      "body": "GitHub에서 Claude Cowork를 다운로드한 후, 원하는 마케팅 워크플로우를 스킬로 정의하여 Cowork에 연결한다. 분석 자료, 데이터, 시장 조사 결과를 Cowork에 입력하고 PPT 스킬을 실행하면 Claude가 자동으로 구조화된 슬라이드 덱을 생성한다. 단순한 템플릿 채우기가 아니라 내용을 분석하고 논리적 흐름에 맞게 섹션을 구성하는 수준이다.",
      "highlights": ["단순한 템플릿 채우기가 아니라 내용을 분석하고 논리적 흐름에 맞게 섹션을 구성하는 수준이다.", "Visualization, PPT 생성, 데이터 분석 등의 스킬을 추가하면 단 하나의 명령어로 복잡한 마케팅 산출물을 자동으로 생성할 수 있다."],
      "quote": None
    },
    {
      "heading": "마케터를 위한 진입 장벽 낮추기",
    ...
    },
    {
      ...
      "body": "Claude Chat은 대화형 탐색과 일회성 작업에 적합하고, Claude Code는 복잡한 개발 프로젝트에 특화되어 있다. Cowork는 그 사이의 영역 — 반복적이고 구조화된 비개발 작업을 자동화하는 데 최적화되어 있다. 세 인터페이스를 작업 성격에 맞게 조합하여 사용하는 것이 Claude 생태계를 최대한 활용하는 전략이다.",
...
summaries["800+ hours of Learning Claude Code in 8 minutes (2026 tutorial / unknown tricks / newest model)"] = {
  "sections": [
    {
      ...
      "highlights": ["더 복잡하게 사용할수록 더 나쁜 결과를 낳는다는 것이 핵심 교훈이다.", "대부분의 사용자가 하는 실수는 충분한 컨텍스트 없이 작업을 시작하거나 컨텍스트를 오염시키는 것이다."],
    ...
    },
    {
      ...
      "body": "첫째, 결과물이 아닌 의도를 설명한다. 둘째, 금지 사항을 명시한다. 셋째, 예시를 제공한다. 넷째, 단계적으로 검증 요청을 포함한다. 다섯째, 불확실한 부분은 질문하도록 명시적으로 허용한다. 이 5가지 패턴을 일관되게 적용하면 Claude Code의 결과물 품질이 크게 향상된다.",
      "highlights": ["결과물이 아닌 의도를 설명하는 것이 가장 중요한 프롬프트 원칙이다.", "금지 사항을 명시하는 것이 긍정적 지시만큼 중요하다."],
      "quote": None
    },
    {
      "heading": "컨텍스트 관리: 실패를 막는 핵심 습관",
      "body": "컨텍스트 윈도우가 가득 차면 Claude Code의 품질이 급격히 저하된다. 장기 작업에서는 중간에 새 대화를 시작하고 현재 상태와 다음 목표를 명시적으로 리셋하는 것이 필수다. 오류 수정 시 같은 대화에서 계속 시도하는 것보다 새 대화에서 더 명확한 지시로 재시작하는 것이 훨씬 효과적인 경우가 많다.",
    ...
    },
    {
      ...
      "highlights": ["기존 프로젝트에서는 먼저 코드베이스 구조를 충분히 설명하고, 기존 패턴과 일관된 방식으로 작업을 요청해야 한다.", "기존 코드베이스에서 Claude Code가 일관성을 유지하지 못하는 가장 큰 원인은 컨텍스트 부족이다."],
      "quote": None
    },
    {
      "heading": "최신 모델 활용: 모델 특성에 맞게 CLAUDE.md 재검토",
      "body": "새 모델(Claude 3.7 Sonnet 등)이 출시될 때마다 이전에 개발했던 CLAUDE.md 설정을 최신 모델 특성에 맞게 재검토하는 것이 필요하다. 새 모델은 이전 모델과 다른 강점과 약점을 가지므로 기존 설정이 최적이 아닐 수 있다. 모델 업데이트 후 작은 테스트를 통해 설정을 조정하는 습관이 장기적으로 일관된 품질을 유지하는 방법이다.",
      "highlights": ["새 모델이 출시될 때마다 CLAUDE.md 설정을 최신 모델 특성에 맞게 재검토하는 것이 필요하다.", "모델 업데이트 후 작은 테스트를 통해 설정을 조정하는 습관이 장기적으로 일관된 품질을 유지하는 방법이다."],
      "quote": None
    }
  ],
  "key_takeaways": [
  ...
  "apply_points": [
    ...
    {"text": "Claude 모델 업데이트 시 각 프로젝트 CLAUDE.md 설정을 재검토하는 루틴을 orchestration workflow에 추가", "key": True}
  ],
  "smart_brevity": {
...
# The Nano Banana, Designer, Google Stitch, NotebookLM summaries are identical to what was
# defined in wim_gen_0321.py — copy them here exactly

summaries["Claude Code + Nano Banana 2 + Kling =  $15K Animated Sites"] = {
  "sections": [
    {
      "heading": "15분에 $15,000 수준의 애니메이션 웹사이트 제작",
      "body": "Nano Banana 2(이미지 생성), Kling(비디오 변환), Claude Code(코딩)를 조합하면 과거에는 5,000~10,000달러가 들었던 3D 스크롤 애니메이션 웹사이트를 15분, 2~3달러의 비용으로 제작할 수 있다. 헤드폰, 환경 보호 지구본, 인테리어 디자인, 우주 정거장 등 다양한 테마의 웹사이트 4개를 시연하며 이 도구 조합의 실용성을 증명한다. 핵심은 각 도구의 역할 분리다 — Nano Banana 2로 시작 이미지 생성, Kling으로 영상 변환, Claude Code로 웹사이트 조립.",
      "highlights": ["과거에는 5,000~10,000달러가 들었던 3D 스크롤 애니메이션 웹사이트를 15분, 2~3달러의 비용으로 제작할 수 있다.", "핵심은 각 도구의 역할 분리 — Nano Banana 2로 시작 이미지 생성, Kling으로 영상 변환, Claude Code로 웹사이트 조립."],
...
summaries["Use Claude Code DESIGNER Skill to 10x UI Designs"] = {
  "sections": [
    ...
    },
    {
      ...
      "body": "Claude Code에 DESIGNER 스킬을 추가하면 일반 코딩 어시스턴트가 UI 디자인 전문가 수준으로 업그레이드된다. 이 스킬은 Claude Code가 디자인 원칙, 시각적 계층 구조, 타이포그래피, 색상 이론을 이해하고 적용하도록 시스템 프롬프트를 강화한다. 스킬 파일에 특정 디자인 스타일 가이드나 브랜드 토큰을 추가하면 일관된 브랜드 아이덴티티를 유지하는 UI 생성이 가능해진다.",
      "highlights": ["DESIGNER 스킬은 Claude Code가 디자인 원칙, 시각적 계층 구조, 타이포그래피, 색상 이론을 이해하고 적용하도록 시스템 프롬프트를 강화한다.", "스킬 파일에 특정 디자인 스타일 가이드나 브랜드 토큰을 추가하면 일관된 브랜드 아이덴티티를 유지하는 UI 생성이 가능해진다."],
      "quote": None
    },
    {
      ...
      "highlights": ["디자인 의도, 타겟 사용자, 원하는 분위기, 시각적 레퍼런스를 함께 제공하는 것은 결과물 품질에 큰 차이를 만든다.", "DESIGNER 스킬이 활성화된 Claude Code는 폰트 선택, 여백 설계, 컬러 팔레트에서 더 섬세한 결정을 내린다."],
      "quote": None
    },
    {
      "heading": "스킬 파일 구조와 팀 표준화",
      "body": "DESIGNER 스킬 파일은 Claude Code가 UI를 생성할 때 따라야 하는 디자인 원칙, 검토 체크리스트, 피해야 할 반패턴을 정의한다. 이 구조는 팀 전체가 동일한 디자인 기준으로 AI를 활용할 수 있게 하는 표준화 메커니즘이기도 하다. Claude Code에 스킬 파일을 추가하고 헤더에 활성화하면 즉시 사용 가능하다.",
      "highlights": ["이 구조는 팀 전체가 동일한 디자인 기준으로 AI를 활용할 수 있게 하는 표준화 메커니즘이기도 하다.", "Claude Code에 스킬 파일을 추가하고 헤더에 활성화하면 즉시 사용 가능하다."],
      "quote": None
    }
  ],
  "key_takeaways": [
...
summaries["Claude Code + Nano Banana = Beautiful Animated Websites"] = {
  "sections": [
    {
      ...
      "body": "AI로 만든 웹사이트의 가장 큰 문제는 어디서나 볼 수 있는 제너릭한 디자인, 즉 AI 슬롯 몬스터 현상이다. 이를 극복하는 가장 강력하고 간단한 방법이 애니메이션 추가인데, 99%의 AI 생성 웹사이트에 없는 요소이므로 즉각적인 차별화가 가능하다. Nano Banana Pro + Kling + Claude Code 조합으로 이 차별화를 구현하면 단순하지만 임팩트 큰 솔루션이 된다.",
    ...
    {
      "heading": "워크플로우: 시작 이미지 → 비디오 → 웹사이트",
      "body": "Nano Banana Pro에서 시작 이미지를 생성하고, Kling에서 루프 가능한 배경 비디오로 변환한 후, Claude Code로 웹사이트를 완성하는 3단계 워크플로우다. 시작 이미지는 비디오로 변환될 첫 번째 프레임이 되므로 움직임이 자연스럽게 보일 수 있는 배경 구도가 중요하다. Claude Code는 비디오 배경 위에 오버레이되는 텍스트, 버튼, 섹션을 생성하며 비디오-텍스트 간 가독성 처리도 자동으로 수행한다.",
      "highlights": ["시작 이미지는 비디오로 변환될 첫 번째 프레임이 되므로 움직임이 자연스럽게 보일 수 있는 배경 구도가 중요하다.", "Claude Code는 비디오 배경 위에 오버레이되는 텍스트, 버튼, 섹션을 생성하며 비디오-텍스트 간 가독성 처리도 자동으로 수행한다."],
      "quote": None
    },
    {
      "heading": "Kling: 이미지를 생동감 있는 루프 비디오로",
      "body": "Kling은 이미지의 시각적 요소에 카메라 패닝, 패럴랙스 효과, 환경 움직임을 추가하여 생동감 있는 배경을 만들어낸다. 비디오 길이와 루프 설정을 웹사이트 배경에 적합하게 조정하는 것이 마지막 단계다. 이 단계가 정적 웹사이트를 역동적으로 만드는 핵심 변환점이다.",
      "highlights": ["Kling은 이미지의 시각적 요소에 카메라 패닝, 패럴랙스 효과, 환경 움직임을 추가하여 생동감 있는 배경을 만들어낸다.", "이 단계가 정적 웹사이트를 역동적으로 만드는 핵심 변환점이다."],
      "quote": None
    },
    {
      "heading": "시작 이미지 품질이 최종 결과를 결정한다",
      "body": "같은 도구를 써도 시작 이미지의 품질과 구도가 최종 웹사이트의 시각적 수준을 결정한다. 색상 팔레트, 분위기, 움직임 가능성을 고려한 시작 이미지 생성에 충분한 시간을 투자하는 것이 전체 결과물 품질을 좌우한다. 이 원칙은 단순히 이미지 생성뿐 아니라 AI 워크플로우 전반에서 첫 번째 단계의 품질이 이후 모든 단계를 결정한다는 보편적 교훈이기도 하다.",
      "highlights": ["같은 도구를 써도 시작 이미지의 품질과 구도가 최종 웹사이트의 시각적 수준을 결정한다.", "이 원칙은 AI 워크플로우 전반에서 첫 번째 단계의 품질이 이후 모든 단계를 결정한다는 보편적 교훈이기도 하다."],
      "quote": None
    }
  ],
  "key_takeaways": [
  ...
  "apply_points": [
    ...
    {"text": "AI 워크플로우에서 첫 번째 단계 품질이 이후 모든 단계를 결정한다는 원칙은 orchestration 파이프라인 설계에도 적용됨", "key": False},
...
summaries["Google Stitch + Claude Code = Insane App Design"] = {
  "sections": [
    ...
    {
      "heading": "design.md: 구조화된 디자인 명세서",
      ...
      "highlights": ["이 파일은 단순한 문서가 아니라 Claude Code가 해석하고 실행할 수 있는 구조화된 디자인 명세서다.", "기존 앱의 디자인 언어를 Stitch에 업로드하면 역방향으로 design.md를 추출할 수도 있다."],
      "quote": None
    },
    {
      "heading": "design.md + Claude Code: 앱 UI 자동 업데이트 워크플로우",
      "body": "Stitch에서 내보낸 design.md를 프로젝트에 추가하고, Claude Code에게 이 파일을 참조하여 UI를 업데이트하도록 지시하면 전체 앱의 디자인이 일관되게 변경된다. design.md가 변경되면 Claude Code가 앱 전체에서 해당 스타일이 적용된 모든 컴포넌트를 찾아 일관되게 업데이트한다. 이는 디자인 변경사항이 즉시 코드에 반영되는 진정한 디자인-코드 동기화를 실현한다.",
    ...
    },
    {
      ...
      "body": "기존 AI 슬롯 외형의 앱을 Stitch로 새 디자인 시스템을 생성하고, design.md로 내보내 Claude Code와 연동하면 앱 전체가 새 디자인으로 업데이트되는 과정을 시연한다. 색상, 폰트, 버튼 스타일, 카드 레이아웃 등 모든 시각적 요소가 design.md에 정의된 스타일로 일관되게 교체된다. 디자인 의도가 코드 레벨에서 명시적으로 관리되므로 AI가 임의로 디자인 결정을 내리는 것을 구조적으로 방지한다.",
      "highlights": ["색상, 폰트, 버튼 스타일, 카드 레이아웃 등 모든 시각적 요소가 design.md에 정의된 스타일로 일관되게 교체된다.", "디자인 의도가 코드 레벨에서 명시적으로 관리되므로 AI가 임의로 디자인 결정을 내리는 것을 구조적으로 방지한다."],
      "quote": None
    },
    {
      "heading": "AI 슬롯 탈출 전략: Stitch의 차별화 포인트",
      "body": "Google Stitch가 다른 AI 디자인 도구와 다른 점은 시각적 프리셋이 아니라 구조화된 디자인 시스템을 코드 형태로 정의한다는 것이다. 이 접근법은 AI가 만드는 모든 UI가 동일한 디자인 기준을 따르도록 강제하여, 반복 사용할수록 브랜드 일관성이 강화된다. design.md가 코드베이스의 표준 파일이 되면 AI는 항상 디자이너의 의도를 먼저 읽고 UI를 결정하게 된다.",
      "highlights": ["Google Stitch는 시각적 프리셋이 아니라 구조화된 디자인 시스템을 코드 형태로 정의한다.", "design.md가 코드베이스의 표준 파일이 되면 AI는 항상 디자이너의 의도를 먼저 읽고 UI를 결정하게 된다."],
      "quote": None
    }
  ],
  "key_takeaways": [
...
summaries["Nano Banana 2: The JSON Control Hack"] = {
  "sections": [
    ...
    },
    {
      ...
      "body": "간단한 색상 변경부터 시작하여 복잡한 원근감 조정, 오브젝트 교체까지 JSON 포맷의 제어력이 어떻게 작동하는지 단계적으로 시연한다. 색상 변경 JSON은 target_object, change_color, preserve_elements를 명시하고, 오브젝트 교체는 replace_object, maintain_perspective, keep_unchanged_elements를 정의한다. 원근감을 유지하면서 오브젝트만 교체하는 가장 어려운 유스케이스에서 JSON 포맷이 가장 큰 차이를 만든다.",
    ...
    {
      "heading": "Nano Banana 2와 JSON의 시너지",
...
summaries["NotebookLM is becoming the Most Essential AI Tool (7 Use Cases to Try!)"] = {
  "sections": [
    ...
    {
      "heading": "유스케이스 1-2: 전략 프레젠테이션 자동 준비와 팟캐스트 생성",
      "body": "30개 이상의 산업 보고서를 읽을 시간이 없을 때 NotebookLM에 소스를 업로드하고 전략 질문을 입력하면 수분 내에 프레젠테이션 자료가 준비된다. Forrester, Gartner 같은 권위 있는 출처를 우선시하도록 프롬프트를 통해 제어할 수 있다. Audio Overview 기능으로 업로드된 소스들을 바탕으로 대화형 팟캐스트를 자동 생성하며, 팟캐스트 진행 중 실시간 인터럽트 질문도 지원한다.",
      "highlights": ["Forrester, Gartner 같은 권위 있는 출처를 우선시하도록 프롬프트를 통해 제어할 수 있다.", "Audio Overview 기능으로 업로드된 소스들을 바탕으로 대화형 팟캐스트를 자동 생성하며, 팟캐스트 진행 중 실시간 인터럽트 질문도 지원한다."],
      "quote": None
    },
    {
      "heading": "유스케이스 3-5: 콘텐츠 변환, 교육 자료, 팀 지식 공유",
      "body": "Studio 패널의 원클릭 변환으로 동일한 소스에서 인포그래픽, 뉴스레터, 소셜 미디어 포스트를 병렬로 생성할 수 있어 콘텐츠 마케팅 워크플로우가 단순화된다. 학습 자료를 업로드하고 플래시카드, 퀴즈, 학습 가이드를 자동 생성하면 교육 콘텐츠 제작 시간을 크게 단축할 수 있다. 팀 노트북을 공유하면 모든 팀원이 같은 소스를 기반으로 질문하고 답변을 얻을 수 있어 지식 분산 문제를 해결한다.",
      "highlights": ["Studio 패널의 원클릭 변환으로 동일한 소스에서 복수의 콘텐츠 형태를 병렬로 생성할 수 있어 콘텐츠 마케팅 워크플로우가 단순화된다.", "팀 노트북을 공유하면 모든 팀원이 같은 소스를 기반으로 질문하고 답변을 얻을 수 있어 지식 분산 문제를 해결한다."],
      "quote": None
    },
    {
      "heading": "유스케이스 6-7: 기업 온보딩과 리서치 심화",
      "body": "기업 내부 문서, 프로젝트 히스토리, 정책 문서를 노트북에 통합하면 신규 팀원 온보딩 시간을 극적으로 단축할 수 있다. fast research(빠른 소스 스캔)와 deep research(전체 분석 리포트)를 목적에 따라 선택하여 사용하면 시간 대비 리서치 깊이를 최적화할 수 있다. 리서치 초점, 신뢰할 수 있는 출처, 시간 범위를 프롬프트로 명시하면 더 정밀한 리서치 결과를 얻을 수 있다.",
      "highlights": ["기업 내부 문서를 노트북에 통합하면 신규 팀원 온보딩 시간을 극적으로 단축할 수 있다.", "fast research와 deep research를 목적에 따라 선택하여 사용하면 시간 대비 리서치 깊이를 최적화할 수 있다."],
      "quote": None
    }
  ],
  "key_takeaways": [
...
print(f"Done batch 2c: {count} updated")
