...
**Why it matters:** Context Engineering의 Gate A(직접 읽기)에서 1M 토큰 전체를 균일 요금으로 쓸 수 있게 됐다. 900K 토큰 요청도 9K와 동일 요율이면, Compact 점진화(500K/700K/900K)의 비용 압박이 크게 완화된다.
...
**Why it matters:** Claude Code ARR 25억 달러, 기업 시장점유율 40%는 orchestration이 의존하는 생태계가 빠르게 성장 중이라는 신호다. Accenture 30,000명이 Claude를 사용하면, MCP 서버와 에이전트 워크플로우 관련 도구·문서가 풍부해진다.
...
**Why it matters:** 675B 파라미터 중 41B만 활성화하는 MoE 구조가 Apache 2.0으로 공개되면, 멀티AI 협업에서 Codex/Gemini 대안으로 로컬 실행 가능한 고성능 모델이 추가된다. 3B~675B 크기 옵션은 역할별(설계/추출/커밋) 최적 모델 선택을 가능하게 한다.
