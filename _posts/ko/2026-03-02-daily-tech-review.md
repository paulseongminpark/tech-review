...
**Why it matters:** Anthropic이 안전장치 해제를 거부한 것은, orchestration이 Claude를 유일한 설계/결정권자로 쓰는 이유와 직결된다. 안전 원칙을 정부 압력에도 굽히지 않는 모델 제공자를 신뢰할 수 있다는 것이 시스템 신뢰의 기반이다.
...
**Why it matters:** 16백만 건의 Claude 대화가 추출됐다는 것은, orchestration이 의존하는 Claude의 추론·도구 사용 패턴이 복제될 수 있다는 의미다. API 사용 패턴 자체가 지적 재산이 되는 시대에, 증류 방어가 모델 제공자의 필수 역량이 됐다.
...
**Why it matters:** Opus 4.6이 에이전틱 코딩 1위, Sonnet 4.6이 이전 Opus급 성능을 저가로 제공하면, orchestration에서 Workers 역할 배분이 최적화된다. Opus(설계/결정) + Sonnet(compressor) + Haiku(commit-writer) 계층 구조의 비용 효율이 극적으로 개선된다.
